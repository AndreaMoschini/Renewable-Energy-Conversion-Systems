...
omega_s=acos(-tan(phi)*tan(delta)); % sunrise/sunset angle at the reference location [rad]
...
for i=1:12 % loop over months
    ...
    cosThetaz=cos(omega)*cos(delta(i))*cos(phi)+sin(delta(i))*sin(phi); % cos of azimuth angle (wrt horizontal)
    % cosThetai= 1; %cos(beta)*cosThetaz+sin(beta)*(sin(phi)*cos(gamma)*cos(omega)*cos(delta(i))+sin(gamma)*sin(omega)*cos(delta(i))-sin(delta(i))*cos(phi)*cos(gamma));
    
    Rt=1./cosThetaz;
    beta = acos(cosThetaz); % beta = Theta_z
    % Ti find gamma, solve cos(theta) = 1 wrt gamma
    gamma = fsolve(@(g)(-1+cos(beta).*(sin(delta(i))*sin(phi) + cos(delta(i))*cos(omega)*cos(phi)) - sin(beta).*(cos(g).*(cos(phi)*sin(delta(i)) - cos(delta(i))*cos(omega)*sin(phi)) - cos(delta(i))*sin(g).*sin(omega)) ),0*beta);
    ...
    Voc=Voc0*(1+chi_oc/100*(Tm-25)); % [V] tensione a vuoto
    Power=N*Isc.*Voc*FF; % Watt 
    Energy(i) = trapz(time,Power); % Wh
...
grid on
xlabel('time (h)','fontsize',12,'fontname','times new roman')
ylabel('\it\beta (°)','fontsize',12,'fontname','times new roman')
legend('data','correlation')
set(gca,'fontsize',12,'fontname','times new roman')
legend('jan.','feb.','mar.','apr.','may','jun.','jul.','aug.','sep.','oct.','nov.','dic.')
subplot(1,2,2)
grid on
xlabel('time (h)','fontsize',12,'fontname','times new roman')
ylabel('\it\gamma (°)','fontsize',12,'fontname','times new roman')
legend('data','correlation')
set(gca,'fontsize',12,'fontname','times new roman')
legend('jan.','feb.','mar.','apr.','may','jun.','jul.','aug.','sep.','oct.','nov.','dic.')
